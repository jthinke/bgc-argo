...
plot_figure_4<-function(){
  ...
  df = dplyr::select(dat, id, date, lq, lon, lat)
  ...
  fit <- fit_ssm(df,
                 vmax = 2.77778,   # max travel rate (m/s) = 10 km /h
                 min.dt = 30,      # minimum allowable time difference between observations (in minutes)
                 model = "rw", 
                 time.step = 2,   # the regular time interval, in hours to predict to
                 verbose = 1)     
  ...
  fig.FG.argos = ggplot() + 
    ...
    layer_spatial(FG.sf_locs, size = 0.75, alpha = 0.1, aes()) +  # specified stage as ID above
