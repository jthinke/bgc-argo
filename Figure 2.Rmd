...
plot_figure_2<-function(){
  ...
  ice_layer<-paste(my_wd, "/data/ice2017_6", sep="")
  mask_layer<-paste(my_wd, "/data/gshhg_shp/", sep="")
  ...
  for(i in 1:2){
    #
    panels<-c("a","b")
    fileName<-paste("Figure_2", panels[i], ".pdf", sep="")
    ...
    longs<-seq(from=-180, by=20, to=180)
    lats<-seq(from=-80, by=5, to=-45)
    nlongs<-length(longs)
    nlats<-length(lats)
    ...
    plot(ant$geometry, axes=FALSE, col="gray10", add=TRUE, lwd=0.1)
    plot(isl$geometry, axes=FALSE, col="gray10", add=TRUE, lwd=0.1)
    ...
    legend(x=500000, y=480000,
           ...
           pt.cex=1,
           bty="n")
    savePlot(filename=fileName, type="pdf")
  }
  ...
  for(i in 1:2){
    ...
    fileName<-paste("Figure_2", panels[i], ".pdf", sep="")
  #windows()
  colx<-colorRampPalette(c("white","red"))
  coly<-colorRampPalette(c("lightblue","navy"))
  ...
  ice.col<-"aquamarine"
  plot(ice$geometry, col=ice.col, border=ice.col, add=TRUE)
  plot(ant$geometry, col="black", add=TRUE)
  plot(isl$geometry, col="black", add=TRUE)
